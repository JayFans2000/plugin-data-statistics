    ...
    public Mono<PieChartVO> getPieChartVO() {
        ...
        Mono<List<PieChartVO.Article>> articlesMono = client.listAll(Post.class, new ListOptions(),
            ...
            .map(posts -> {
                ...
                LocalDate startDate = LocalDate.of(today.getYear() - 1, 1, 1);
                
                // 生成完整日期范围，没有数据的日期默认为0
            ...
            .map(comments -> {
                ...
                return commentsByUser.entrySet().stream()
                    .map(entry -> {
                        ...
                        commentVO.setName(firstComment.getSpec().getContent());
                        commentVO.setEmail(entry.getKey()); // 使用用户名作为标识
                        commentVO.setUsername(entry.getKey());
